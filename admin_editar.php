<?php
	...
	************************************************************/
	...
	function formLoginShow(){
	    ...
	    $respuesta->Assign("loginform","innerHTML",$form);
	    return $respuesta;
	}
	...
	************************************************************/
	...
	function checkDataForm($form){
	    ...
	    $check["Error"]=0;
	    if($form["pNombre"]==""){
	        $check["Msg"]="Ingrese Primer Nombre";
	        $check["Error"]=1;
	    }
	
	    elseif($form["apellido"]==""){
	        $check["Msg"]="Ingrese Apellido Paterno";
	        $check["Error"]=1;
	    }
	    return $check;
	}
	...
	function verificaUsuarioShow($form_entrada){
        ...
        switch($NroRegistros){
			...
			case 1:
				...
				$_SESSION["area_description"]=$area_description;
	...
	*******************************************/
	function menuShow($sessionidarea=0){
	    ...
	    $menu.="<div><h3 class='txt-rojo'>Consultas :</h3>
                    <div class='submenu'>»<a class='negro' href='#'  onClick='xajax_formConsultaShow(\"$idfrom\",\"admin\",\"$idarea\"); return false'>B&uacute;squeda y Edici&oacute;n</a></div>
...
/*****************************************************/                
		...
		if($tipo==0){
			...
			$objResponse->assign("titulo1","innerHTML","<a href=#1 onclick=\"xajax_displaydiv('titulo_resumen','titulo1'); return false;\">T&iacute;tulo / Resumen</a>");		    
                        ...
                        //###############################################################
			//PARA EL AUTOR
                        $objResponse->assign("titulo2","innerHTML","<a href=#1 onclick=\"xajax_displaydiv('author','titulo2'); return false;\">Autor2</a>");
                        $objResponse->script("xajax_searchAuthorSesionPriShow()");
                        $objResponse->script("xajax_searchAuthorSesionSecShow()");
                        $objResponse->assign("search_authorPRI","innerHTML",iniAuthorPriShow());    		
			$objResponse->assign("newFormAuthor","innerHTML",formAuthorShow());
			...
			// ARCHIVO			
                        ...
                        $objResponse->alert(print_r($tipo))
		}
		else if($tipo==3){
                        $funcion="newPublication";
			$objResponse->assign("idcontactform","style.display","block");
	...
	function formConsultaShow($idbutton,$seccion="",$idarea=0){
		...
		<div>
			...
			<form id="formSearch">
				...
				<div>
                                    <div id="div_titulo" class="campo-buscador">IDDATA:&nbsp;</div>
                                    <div id="div_tituloSearch" class="contenedor-caja-buscador-1">
                                        <input id="iddata" name="iddata" type="text" size="30" class="caja-buscador-1">
                                    </div>
				</div>
                                <div style="clear:both;"></div>
				...
				<div style="float:right"><input id="botonbuscar" class="ui-state-default ui-corner-all" onclick='.$functionButton.' type="button" value="Buscar"></div>
                                    '.$formArea.'
				<div class="campo-buscador">Buscar en :</div>
				<div class="contenedor-combo-buscador-1" id="divCategory">
					'.$comboTipoPublicacion.'						
				</div>					
				<div style="clear:both;"></div>
				<div>
                                    <div id="div_titulo" class="campo-buscador">T&iacute;tulo :&nbsp;</div>
                                    <div id="div_tituloSearch" class="contenedor-caja-buscador-1">
